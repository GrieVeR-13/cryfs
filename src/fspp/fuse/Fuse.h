...
public:
  ...
  FuseSession * runInBackground(const boost::filesystem::path &mountdir, std::vector<std::string> fuseOptions);
  FuseSession * runInForeground(const boost::filesystem::path &mountdir, std::vector<std::string> fuseOptions);
  bool running() const;
  void stop();
  ...
  int utimens(const boost::filesystem::path &path, timespec lastAccessTime, timespec lastModificationTime);
...
private:
  ...
  FuseSession * _run(const boost::filesystem::path &mountdir, std::vector<std::string> fuseOptions);
