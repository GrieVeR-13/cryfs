...
namespace cryfs_cli {
    ...
    public:
        ...
        FuseSession *main(jobject pathnameFileSystem, int argc, const char **argv, std::function<void()> onMounted);

    private:
        void _checkForUpdates(cpputils::unique_ref<cpputils::HttpClient> httpClient);
        FuseSession *_runFilesystem(const program_options::ProgramOptions &options, std::function<void()> onMounted);
        ...
        cpputils::FsAndPath _determineConfigFile(const program_options::ProgramOptions &options);
